...
- **Трекинг поведения** — переключение вкладок, копирование/вставка, потеря фокуса окна, открытие DevTools, уход курсора за пределы окна; всё на таймлайне с воспроизведением ответов.
- **Полноэкранный режим** — экзамен идёт в полном экране, выход фиксируется. Тумблер в настройках экзамена (по умолчанию вкл); при включённом — **обязателен для сдачи**.
- **Запись экрана** — опционально (тумблер): периодические скриншоты экрана студента с настраиваемым интервалом. Требует шаринга **всего экрана** и **HTTPS** (secure context). У преподавателя — галерея кадров с навигацией (стрелки / клик по левой-правой половине).
